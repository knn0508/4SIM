...
def read_population_territory_data():
    """Read population and territory data from Excel file"""
    try:
        # Read the Excel file
        df_raw = pd.read_excel('Ehali_ve_Erazi.xlsx', header=None)

        print(f"Excel file loaded. Shape: {df_raw.shape}")

        # Try to find the row with year 1970 in any column
        start_row = None
        for i in range(len(df_raw)):
            for j in range(df_raw.shape[1]):
                try:
                    cell_value = df_raw.iloc[i, j]
                    # Check if the cell contains 1970
                    if str(cell_value).strip() == '1970':
                        start_row = i
                        break
                except:
                    continue
        ...
        if start_row is None:
            print("Could not find the starting row with year 1970")
            return None
        ...
        data_rows = data_rows.iloc[:, 1:].copy()

        # Define column names based on the structure
        column_names = [
            'Year', 'Əhali (min nəfər)', 'Ərazi (min km²)', 'Kənd təsərrüfatına yararlı torpaqlar (min ha)',
            '1 km² əraziyə adam düşür (nəfər)', '100 ha-ya adam düşür (nəfər)',
            'Adambaşına kənd təsərrüfatına yararlı torpaq (ha)',
            'Əkin yeri (min ha)', 'Adambaşına əkin yeri (ha)', 'Dincə qoyulmuş torpaqlar (min ha)',
            'Çoxillik əkmələr (min ha)', 'Biçənək və örüş-otlaq sahələri (min ha)', 'Meşə ilə örtülü sahələr (min ha)'
        ]

        if data_rows.shape[1] == len(column_names):
            data_rows.columns = column_names
        else:
            column_names = column_names[:data_rows.shape[1]]
            data_rows.columns = column_names

        # Clean and filter data - CHANGED: Filter for 1990-2023 instead of 1970-2023
        data_rows = data_rows.dropna(subset=['Year'])
        data_rows['Year'] = pd.to_numeric(data_rows['Year'], errors='coerce')
        data_rows = data_rows.dropna(subset=['Year'])
        data_rows = data_rows[(data_rows['Year'] >= 1990) & (data_rows['Year'] <= 2023)]

        # Convert Year to datetime and set as index
        data_rows['Date'] = pd.to_datetime(data_rows['Year'], format='%Y')
        data_rows.set_index('Date', inplace=True)
        data_rows.drop('Year', axis=1, inplace=True)

        # Convert all numeric columns to float, handling commas
        for col in data_rows.columns:
            if data_rows[col].dtype == 'object':
                data_rows[col] = data_rows[col].astype(str).str.replace(',', '.', regex=False)
            data_rows[col] = pd.to_numeric(data_rows[col], errors='coerce')

        # ** FIX: Reindex the DataFrame to ensure a consistent annual frequency **
        # Remove duplicate years if any, keeping the first entry
        if not data_rows.index.is_unique:
            data_rows = data_rows[~data_rows.index.duplicated(keep='first')]

        # Create a complete, regular annual index from the first to the last year
        if not data_rows.empty:
            # Fix: Define start_year and end_year from the actual data
            start_year = int(data_rows.index.min().year)
            end_year = int(data_rows.index.max().year)

            # Use a clean date range with a defined yearly frequency
            full_index = pd.date_range(start=f'{start_year}', end=f'{end_year}', freq='AS-JAN')
            df = data_rows.reindex(full_index)
        else:
            df = data_rows

        # Now, interpolate missing values (for years that were missing in the original data)
        df = df.interpolate(method='linear')

        if len(df) > 0:
            print(f"Data loaded successfully: {len(df)} records from {df.index[0].year} to {df.index[-1].year}")
...
def draw_summary_figure(pdf, population_data, categories_2023, pie_labels, pie_sizes, growth_rates):
    ...
    ax1.set_title('Ümumi Əhali (1970-2023)', fontsize=12, fontweight='bold', pad=8)
    ...
    ax4.set_title('1970-2023 Səmərəlilik və Artım Göstəriciləri (%)', fontsize=12, fontweight='bold', pad=6)
